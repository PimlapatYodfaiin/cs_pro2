...
while ($row = mysqli_fetch_assoc($result)) {
    if(date("Y-m-d") >= $row['eq_date_calculate'] && $row['eq_price'] > 1){
        if((strtotime(date("Y-m-d")) - strtotime($row['eq_buydate'])) >= 365){  //ครบปี
...
// $depreciation = round('eq_price'- 1, 2);
// $value    = round($depreciation/'type_lifetime_low', 2);

// $value2 = round('eq_price' - $value, 2);
